...
**不要等 /clear 或会话结束。** 用户可能随时清屏，你来不及反应。

每完成一个实质性任务（修好 bug、做完功能、做出决定、改完配置），立即更新 `memory/_bridge.md`：

- 新任务开始时 → 更新「开了什么坑」
- 做完一个 → 从坑里划掉，更新「做了什么决定」
- 遇到堵塞 → 更新「堵塞」
- 任何时候优先级变了 → 更新「下一步」
- 每写完 _bridge.md → 同步更新 _last-context.md（时间戳+摘要）

**铁律**：_bridge.md 永远不能超过一个任务陈旧。用户 /clear 那一刻，最多丢当前这一个任务的上下文。
